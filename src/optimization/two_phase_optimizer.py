...
class TwoPhaseExtensiveFormOptimizer:
    ...
    def solve(
        ...
        # ── Write LP file so gurobipy can read it directly ─────────────────
        import tempfile, os
        lp_path = os.path.join(tempfile.gettempdir(), "two_phase_model.lp")
        model.writeLP(lp_path)
        print(f"\n  LP model written: {lp_path}")

        # ── Solve via gurobipy directly (avoids PuLP's broken status mapping) ──
        try:
            ...
            env.setParam("LogFile",     "")   # suppress logfile creation
            grb = gp.read(lp_path, env)
            ver = ".".join(str(v) for v in gp.gurobi.version())
            print(f"  ✓ Gurobi {ver} (gurobipy direct — status 9 safe)")
            ...
            grb_status = grb.status   # 2=Optimal, 9=TimeLimitFeasible, 3=Infeasible
            sol_count  = grb.SolCount

            FEASIBLE_CODES = {GRB.OPTIMAL, GRB.TIME_LIMIT,
                              GRB.INTERRUPTED, GRB.SUBOPTIMAL, 13}

            if grb_status in FEASIBLE_CODES and sol_count > 0:
                obj_val   = grb.ObjVal
                obj_bound = grb.ObjBound
                gap_pct   = abs(obj_val - obj_bound) / max(abs(obj_val), 1e-10) * 100
                status    = "Optimal" if grb_status == GRB.OPTIMAL else "Feasible"
                print(f"  Status   : {status}  ({elapsed:.1f}s)")
                print(f"  Objective: {obj_val:,.0f} VND  "
                      f"[bound={obj_bound:,.0f}  gap={gap_pct:.2f}%]")

                # Map Gurobi solution values back into PuLP variable objects
                grb_var_map = {v.VarName: v for v in grb.getVars()}
                for lp_var in model.variables():
                    gv = grb_var_map.get(lp_var.name)
                    lp_var.varValue = gv.X if gv is not None else 0.0

            else:
                status = "Infeasible" if grb_status == GRB.INFEASIBLE else "Not Solved"
                print(f"  Status   : {status} (Gurobi code={grb_status}, "
                      f"solutions={sol_count})")
                return status, {}

        except ImportError:
            # ── CBC fallback if gurobipy is not importable ─────────────────
            print("  ⚠ gurobipy not importable → falling back to CBC solver.")
            solver = PULP_CBC_CMD(msg=True, timeLimit=time_limit, gapRel=gap_tolerance)
            print(f"\nSolving with CBC (limit={time_limit}s, gap={gap_tolerance*100:.0f}%)…")
            t0 = time.time()
            model.solve(solver)
            elapsed  = time.time() - t0
            ...
            if status == "Not Solved" and obj_val is not None and obj_val > 0:
                print("  ⚠ CBC hit Time Limit — extracted feasible incumbent.")
                status = "Feasible"
            print(f"  Status: {status}  ({elapsed:.1f}s)")
            if status not in ("Optimal", "Feasible"):
                return status, {}

        # ── Extract solution (reads PuLP var.varValue set above) ───────────
